...
- **小窗 + 置顶。** 以 360×480 窗口运行,顶部有"置顶"按钮可让它始终浮在最上层;专注时窗口自动展开,用系统窗口控件调整大小。不做真正的 OS 全屏(那会隐藏标题栏并占用一个 macOS 空间——对专注小部件来说太重了)。
...
| **单窗口,CSS 自适应** | 一棵 React 树,两套布局。`setSize` 在专注时自动展开;用系统窗口控件调整大小。比两个窗口简单,blob 过渡保持流畅。 |
